...
arima_ts <- function(ts, num_ahead = 5, order = c(1, 0, 0), level = 95)
{
    ...
    forecast_one_step_static(fun = f, ts = ts, num_ahead = num_ahead, 
...
* `arima_ts` is a function that operates on a single time series, with arguments for `num_ahead` (required), and `order` and `level`, which are optional.
  ...
  - `f` is passed to `forecast_one_step_static`, a function which performs the necessary splitting of the data into `training` and `observed` portions, runs `f`, and handles any errors gracefully (returning NAs) if necessary.

We use this (admittedly) convoluted coding structure so that `forecast_one_step_static` provides a consistent wrapper for handling any kind of forecast model, and so that each defined forecasting function (e.g. `arima_ts`) can be used on individual time series, with optional arguments passed in without issue. 
