...
* `arima_ts` is a function that operates on a single time series, with arguments for `num_ahead` (required), and `order` and `level`, which are optional.
  ...
  - `f` is passed to `make_forecasts`, a function which performs the necessary splitting of the data into `training` and `observed` portions, runs `f`, and handles any errors gracefully (returning NAs) if necessary.

We use this (admittedly) convoluted coding structure so that `make_forecasts` provides a consistent wrapper for handling any kind of forecast model, and so that each defined forecasting function (e.g. `arima_ts`) can be used on individual time series, with optional arguments passed in without issue. 
