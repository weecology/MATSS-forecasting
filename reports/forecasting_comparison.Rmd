...
We have quite a few results objects loaded. There is one for each analysis method, combining the individual runs on the different datasets.

```{r}
ls(pattern = results_pattern)
```

Just as in the "autoarima_report", we combine the outputs in each results object using `bind_rows()`, and wrap this all together using `purrr::map_dfr()` (and some cleaning of the dataset names):
